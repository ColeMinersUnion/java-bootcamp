...
public class CustomerValidator {
    ...
    public void validateNew(Customer customer) {
        ...
        if (repository.existsById(customer.getCustomerId())) {
            throw BusinessException.conflict("A customer already exists by that ID", "Duplicate_ID");
        }
        if (repository.existsByEmail(customer.getEmail())) {
            throw BusinessException.conflict("A customer already exists with that email", "Duplicate_Email");
        }
    }
    ...
    public void validateTransition(CustomerStatus from, CustomerStatus to, String correlationId) {
        ...
        if (!allowed.contains(to)) {
            throw new BusinessException("Illegal Transition",
                                        "[" + correlationId + "]: " + from + " to " + to,
                                        440,
                                        correlationId); //arbitrary code
        }
    }
}